    ...
    def __init__(self, method_args):
        ...
        for elem in args:
            if elem in ['flat', 'mean', 'stat']:
                self.feature_type_list.append(elem)
            elif elem in ['none', 'capca', 'pca_50', 'lda']:
                self.reduce_dim_list.append(elem)
            elif elem.isdigit():
                self.alpha_list.append(int(elem))
            else:
